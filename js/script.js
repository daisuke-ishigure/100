...
function updateJsonSelectorOptions() {
  ...
  $("#jsonSelector").html('<option value="all">歌人を選んでください</option>');
  themePoems.forEach(function (poem) {
    ...
    $("#jsonSelector").append(
      `<option value="${poem.number}">第${poem.number}番・${strippedName}</option>`
    );
  });
}
  ...
  fetch(jsonAddress)
    ...
    .then((data) => {
      ...
      $(".explanation").append(`
       <dl>
       <dt>和歌</dt>
       <dd>${first.replace(/<br\s*\/?>/g, " ") }
       <br>${second.replace(/<br\s*\/?>/g, " ") }<br>
       <span class="small">${name}（${date}）</span>
    </dd>
        </dl>
        <dl>
        <dt>現代語訳</dt>
